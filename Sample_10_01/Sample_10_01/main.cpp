...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	while (DispatchWindowMessage())
	{
		...
		// offscreenRenderTargetがレンダリングターゲットとして利用できるまで待つ
		renderContext.WaitUntilToPossibleSetRenderTargets(1, rtArray);
		{
			// レンダリングターゲットを設定（★レンダリングターゲットOMSetRenderTargetsを変更している。あとで戻さないと画面に何も映らなくなってしまう）
		...
		// offscreenRenderTargetへの書き込み終了待ち
		renderContext.WaitUntilFinishDrawingToRenderTargets(1, rtArray);

		{
			// step-6 画面に表示されるレンダリングターゲットに戻す（★OMSetRenderTargets）
